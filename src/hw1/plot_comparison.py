...
def _get_predictions(
    ...
    """Run all models on one window and return predictions dict."""
    mixed_win = (
    ...
    for name, model in models.items():
        model.eval()
        with torch.no_grad():
            preds[name] = model(mixed_win, label).squeeze().cpu().numpy()
    return preds, mixed_win.squeeze().cpu().numpy()
...
def plot_comparison(
    ...
    device    : torch device
    freq_idx  : which frequency to extract (0=1Hz, 1=5Hz, 2=10Hz, 3=20Hz)
    sample_idx: shown in figure title
    ...
    panels = [
        ...
        (axes[2], preds.get("MLP", np.zeros(WINDOW_LEN)), "MLP Prediction"),
        (axes[3], preds.get("RNN", np.zeros(WINDOW_LEN)), "RNN Prediction"),
        (axes[4], preds.get("LSTM", np.zeros(WINDOW_LEN)), "LSTM Prediction"),
    ]
    ...
    axes[5].plot(
        preds.get("MLP", np.zeros(WINDOW_LEN)), label="MLP", linestyle="--", alpha=0.8
    )
    axes[5].plot(
        preds.get("RNN", np.zeros(WINDOW_LEN)), label="RNN", linestyle="-.", alpha=0.8
    )
    axes[5].plot(
        preds.get("LSTM", np.zeros(WINDOW_LEN)), label="LSTM", linestyle=":", alpha=0.8
    )
